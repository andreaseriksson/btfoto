class Cart < ActiveRecord::Base
  ...
  def summary
    ...
    @cart.cart_items.each do |cart_item|
      ...
      products << { id: cart_item.product_id, cart_item_id: cart_item.id,
                    image_nr: cart_item.image_nr, name: cart_item.product.name,
                    price: price, total_price: (price * quantity), quantity: quantity,
                    vat: vat, total_vat: (vat * quantity), discount: nil }
    end
