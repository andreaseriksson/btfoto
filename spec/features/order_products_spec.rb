...
feature 'Order product' do
  ...
  before(:each) do
    @_picture = FactoryGirl.create(:picture)
    @_product = FactoryGirl.create(:product)
  end
  ...
  scenario 'views a product page' do
    ...
    current_path.should eq(product_path(@_product))
    page.should have_content(@_product.description)
  end
